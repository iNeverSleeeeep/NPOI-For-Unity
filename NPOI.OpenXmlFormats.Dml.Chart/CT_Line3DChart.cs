...
namespace NPOI.OpenXmlFormats.Dml.Chart
{
	...
	public class CT_Line3DChart
	{
		...
		public static CT_Line3DChart Parse(XmlNode node, XmlNamespaceManager namespaceManager)
		{
			...
			cT_Line3DChart.ser = new List<CT_LineSer>();
			cT_Line3DChart.axId = new List<CT_UnsignedInt>();
			cT_Line3DChart.extLst = new List<CT_Extension>();
			foreach (XmlNode childNode in node.ChildNodes)
			{
				if (childNode.LocalName == "grouping")
				{
					cT_Line3DChart.grouping = CT_Grouping.Parse(childNode, namespaceManager);
				}
				else if (childNode.LocalName == "varyColors")
				{
					cT_Line3DChart.varyColors = CT_Boolean.Parse(childNode, namespaceManager);
				}
				else if (childNode.LocalName == "dLbls")
				{
					cT_Line3DChart.dLbls = CT_DLbls.Parse(childNode, namespaceManager);
				}
				else if (childNode.LocalName == "dropLines")
				{
					cT_Line3DChart.dropLines = CT_ChartLines.Parse(childNode, namespaceManager);
				}
				else if (childNode.LocalName == "gapDepth")
				{
					cT_Line3DChart.gapDepth = CT_GapAmount.Parse(childNode, namespaceManager);
				}
				else if (childNode.LocalName == "ser")
				{
					cT_Line3DChart.ser.Add(CT_LineSer.Parse(childNode, namespaceManager));
				}
				else if (childNode.LocalName == "axId")
				{
					cT_Line3DChart.axId.Add(CT_UnsignedInt.Parse(childNode, namespaceManager));
				}
				else if (childNode.LocalName == "extLst")
				{
					cT_Line3DChart.extLst.Add(CT_Extension.Parse(childNode, namespaceManager));
				}
			}
